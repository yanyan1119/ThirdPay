...
- (id)initWithResultString:(NSString *)string
{
	if (self = [super init]) {
        ...
        NSError *error;
        NSDictionary *dic = (NSDictionary*)[NSJSONSerialization JSONObjectWithData:srcData options:NSJSONReadingMutableLeaves error:&error];
        
		do {
			NSDictionary * jsonMemo = [dic objectForKey:@"memo"];
			if (jsonMemo == nil)
            {
				break;
			}
            if ([jsonMemo.allKeys containsObject:@"ResultStatus"])
            {
                self.statusCode = [[jsonMemo objectForKey:@"ResultStatus"] intValue];
            }
            else if ([jsonMemo.allKeys containsObject:@"resultStatus"])
            {
                self.statusCode = [[jsonMemo objectForKey:@"resultStatus"] intValue];
            }
			
			self.statusMessage = [jsonMemo objectForKey:@"memo"];
			if (self.statusCode != AlixpayPaySuccess)
            {
				break;
			}
